jamovi_mdiff_initialize <- function(self, grouping_variable = TRUE) {
  ...
  try(tbl_overview <- self$results$overview, silent = TRUE)
  try(tbl_es_mean_difference <- self$results$es_mean_difference, silent = TRUE)
  try(tbl_es_mean_ratio <- self$results$es_mean_ratio, silent = TRUE)
  try(tbl_es_smd <- self$results$es_smd, silent = TRUE)
  try(tbl_es_median_difference <- self$results$es_median_difference, silent = TRUE)
  try(tbl_es_median_ratio <- self$results$es_median_ratio, silent = TRUE)
  try(tbl_es_odds_ratio <- self$results$es_odds_ratio, silent = TRUE)
  try(assume_equal_variance <- self$options$assume_equal_variance, silent = TRUE)
  try(tbl_point_null <- self$results$point_null, silent = TRUE)
  try(tbl_interval_null <- self$results$interval_null, silent = TRUE)
  try(effect_size <- self$options$effect_size, silent = TRUE)
...
jamovi_add_htest_mdiff <- function(self, estimate) {
  ...
  if(evaluate_h) {
    ...
    rope_units <- "raw"
    try(rope_units <- self$options$rope_units, silent = TRUE)
