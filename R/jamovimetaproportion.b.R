...
jamovimetaproportionClass <- if (requireNamespace('jmvcore', quietly=TRUE)) R6::R6Class(
    "jamovimetaproportionClass",
    inherit = jamovimetaproportionBase,
    private = list(
      .init = function() {
...
jamovi_meta_proportion <- function(self) {
  ...
  args$data <- self$data
  args$cases <- unname(self$options$cases)
